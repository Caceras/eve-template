...
export async function runJob(input: JobInput): Promise<JobResult> {
  ...
  if (input.tasks.length === 0) throw new Error("benchmark requires at least one task");
      ...
      `${JSON.stringify(
        {
          ...
          taskEnvironments: input.tasks.map((task) => ({
            task: task.name,
            image: task.environment.dockerImage ?? null,
            source: task.environment.dockerImage ? "prebuilt" : "local-dockerfile",
          })),
