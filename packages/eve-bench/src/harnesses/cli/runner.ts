...
async function main(): Promise<number> {
  ...
  try {
    ...
    const baseEnv = {
      PATH: `${ctx.installDir}/arch:${process.env.PATH ?? "/usr/local/bin:/usr/bin:/bin"}`,
      HOME: home,
      XDG_CONFIG_HOME: `${home}/.config`,
      XDG_CACHE_HOME: `${runtimeDir}/cache`,
      XDG_DATA_HOME: `${runtimeDir}/data`,
      XDG_STATE_HOME: `${runtimeDir}/state`,
      LANG: "C.UTF-8",
      NO_COLOR: "1",
      ...plan.env,
    };
    ...
    if (failure) runtimeResult.failure = failure;
    await writeFile(
      join(ctx.logsDir, "cli-runtime.json"),
