      ...
      config: json({
        providers: {
          "eve-bench-gateway": {
            ...
            models: [
              options.reasoning
                ? { id: ctx.model, reasoning: options.reasoning !== "off" }
                : { id: ctx.model },
            ],
          },
        },
  ...
  if (name === "opencode") {
    return {
      ...
      env: {
        ...
        XDG_DATA_HOME: `${runtimeDir}/opencode/xdg-data`,
        XDG_STATE_HOME: `${runtimeDir}/opencode/xdg-state`,
      },
      configPath: `${runtimeDir}/home/.config/opencode/opencode.json`,
