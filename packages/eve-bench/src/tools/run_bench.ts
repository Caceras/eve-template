...
export default defineTool({
  ...
  async execute(input, ctx) {
    ...
    const tasks = await selectNativeBuild(
      await Promise.all((await taskDirs(datasetDir, selection.tasks)).map((dir) => loadTask(dir))),
      input.nativeBuild,
    );
    ...
    return runJob({
      ...
      forwardEnv: forwardedEnv(harness.credentials ?? []),
