...
describe("createAutoevalsClient", () => {
  ...
  it("adds a grader instruction when a judge request has no system message", async () => {
    mocks.generateText.mockResolvedValueOnce({
      text: "",
      toolCalls: [
        {
    ...
    expect(mocks.generateText).toHaveBeenCalledWith(
      expect.objectContaining({
        messages: expect.arrayContaining([
          {
            content: "You are a grader. Answer by calling the tool you are given.",
            role: "system",
          },
        ]),
      }),
  ...
  it.each(["system", "developer"] as const)(
    "preserves an existing %s message without adding another grader instruction",
    async (role) => {
      mocks.generateText.mockResolvedValueOnce({ text: "ok", toolCalls: [] });
      const client = createAutoevalsClient({ languageModel: "openai/gpt-5.5" });
      ...
      await client.chat.completions.create({
        messages: [
          { content: "Use this rubric.", role },
          { content: "Grade this answer.", role: "user" },
        ],
        model: "openai/gpt-5.5",
      });
      ...
      expect(mocks.generateText).toHaveBeenCalledWith(
        expect.objectContaining({
          messages: [
            { content: "Use this rubric.", role: "system" },
            { content: "Grade this answer.", role: "user" },
          ],
        }),
      );
