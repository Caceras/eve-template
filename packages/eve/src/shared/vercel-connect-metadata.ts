...
export function extractVercelConnectMetadata(value: unknown): VercelConnectMetadata | undefined {
  ...
  const { connector, connectorType, principalTypes, service } = value as {
  ...
  if (connectorType === undefined && principalTypes === undefined && service === undefined) {
    return { connector };
  }
  if (
  ...
  return { connector, connectorType, principalTypes, service };
}
