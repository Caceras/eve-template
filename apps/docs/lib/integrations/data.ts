...
The registry also writes \`agent/schedules/gmail-maintenance.ts\`, which runs \`gmail.watch()\` before \`gmail.sync()\` on a daily \`0 9 * * *\` schedule (09:00 UTC on Vercel). The job registers or renews Gmail's watch, then synchronizes from the saved cursor. Adjust the cadence for your host and plan.
