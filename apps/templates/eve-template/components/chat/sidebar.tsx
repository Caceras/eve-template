...
function SidebarFallback({ className }: { readonly className?: string }) {
  return (
    <aside
      ...
      className={cn(
        "flex h-full w-64 shrink-0 flex-col border-r border-border bg-background px-2 pt-2",
        className,
      )}
    >
      ...
      <nav aria-label="Workspace" className="mt-2 grid gap-0.5">
        {workspacePages.map(({ href, label, icon: Icon }) => (
          <Link
            key={href}
            href={href}
            className="flex min-h-11 items-center gap-2 rounded-lg px-2 text-sm text-muted-foreground md:min-h-9"
          >
