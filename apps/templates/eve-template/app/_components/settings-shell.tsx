...
const SECTIONS: { id: SettingsSection; href: string; label: string; icon: LucideIcon }[] = [
  { id: "general", href: "/settings", label: "Models", icon: CpuIcon },
  { id: "voice", href: "/settings/voice", label: "Voice", icon: MicIcon },
  { id: "notifications", href: "/settings/notifications", label: "App & notifications", icon: BellIcon },
  { id: "integrations", href: "/settings/integrations", label: "Integrations", icon: PlugIcon },
...
const rowClass = "flex h-11 shrink-0 items-center gap-2 rounded-md px-2.5 text-sm transition-colors md:h-9";
...
export function SettingsShell({ section, title, description, actions, children }: {
  ...
  return (
    <div className="h-full overflow-y-auto">
      <div className="mx-auto flex max-w-4xl flex-col gap-6 px-4 pb-16 pt-20 sm:px-8 md:flex-row md:gap-10">
        <nav aria-label="Settings" className="-mx-4 flex gap-1 overflow-x-auto px-4 md:sticky md:top-20 md:mx-0 md:w-48 md:shrink-0 md:flex-col md:self-start md:overflow-visible md:px-0">
          ...
          {SECTIONS.map((item) => (
            <Link aria-current={item.id === section ? "page" : undefined} className={cn(rowClass, item.id === section ? "bg-muted/60 text-foreground" : "text-muted-foreground hover:bg-muted/50 hover:text-foreground")} href={item.href} key={item.id}>
        ...
        <div className="min-w-0 flex-1 md:max-w-2xl">
          ...
          {viewer ? (
            <div className="mt-8 space-y-4">{section !== "security" ? <SecurityNotice /> : null}{children}</div>
          ) : (
