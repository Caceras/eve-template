...
The intended primary host is `aegentica.se`; `ai-chat.se` remains the working alias until DNS, domain routing and TLS are verified. Configure both domains to this same application, not separate processes with diverging data. Verify the `polish-2026-09-24b` health release field after deployment. See [release verification](./RELEASE_VERIFICATION.md) and [agent storage](./AGENTS_AND_ORCHESTRATION.md#storage-and-limits).
