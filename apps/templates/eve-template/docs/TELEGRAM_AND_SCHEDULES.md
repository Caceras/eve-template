...
Ægentica can reach the operator outside the browser. A Telegram bot gives a
private chat with the same agent and memory, and scheduled tasks (reminders,
daily briefs, recurring checks) deliver their results there.
...
Ask in chat: "remind me tomorrow at 9 to call Anna" or "every weekday at 8, send
me a short brief". The agent uses four tools:
...
`eve start`). Each tick it leases due tasks from the encrypted
`settings/schedules.enc` store under a cross-process file lock, then starts a
Telegram session for each with
`to(telegram, { chatId }).send(prompt, { auth: operator })`. Success computes the
next run; a failed hand-off retries after 5 minutes and, after three failures,
moves a recurring task to its next slot. Delivery is at least once.

Tasks wait, rather than fail, while Telegram is not linked. **Settings →
Scheduled tasks** lists every task with its next run and lets the operator pause,
resume or delete it.
...
math, exclusive leases, retries, the Settings APIs (auth and same-origin),
Telegram connect, webhook secret, pairing limits and disconnect.
...
one-minute schedule. Run the app with `FAKE_TELEGRAM_LOG` set and a Node
