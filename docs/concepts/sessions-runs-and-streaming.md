...
A single HTTP response is not held open indefinitely. eve may end a response after a period without events so the server can release its stream reader. This does not stop the run or discard events: reconnect from the last consumed `startIndex` to continue reading the durable stream.
