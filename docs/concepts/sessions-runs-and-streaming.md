...
A single HTTP response is not held open indefinitely. When ten seconds pass without a new event, for example while a tool runs a long sandbox command, eve ends that response normally and releases its server-side reader. Every stream response advertises this interval in the `x-eve-stream-idle-close-ms` header so a client can tell an intentional idle close from a stream that simply ended. The run and the durable stream are unaffected: events written during or after the close are still recorded and are delivered on the next connection. This keeps each stream request bounded on serverless hosts, which may not tell the handler when a client disconnects.
