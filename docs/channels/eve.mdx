...
Create a conversation session before its first message by sending an empty object:
...
The workflow runs session-scoped initialization and parks without starting `turn_0`. Because the
request has no message, it does not invoke `onMessage`; that hook runs when the first message is
sent to the returned session ID. Task mode and turn-scoped fields such as `clientContext` and
`outputSchema` require a message on the create request.

Session-scoped initialization uses the create request's route auth, or the forwarded
identity accepted by the channel. This includes `session.started` handlers, dynamic
definitions, and sandbox setup. The first message's `onMessage` result supplies auth
and context for that turn; it cannot change initialization that already ran or rename
the existing workflow through `title`.

Resolve identity needed during initialization in the channel's `auth` policy and supply
the required credentials or headers on the create request. If initialization depends on
the first message or an auth transformation in `onMessage`, create with a message instead.
Frontend bindings prewarm by default; use `prewarm: false` for those channels.
