...
When `api.url` is omitted, eve reads `SLACK_API_URL`, and otherwise uses `https://slack.com/api/`. The value is normalized to a trailing slash, so `https://sim.example/api` and `https://sim.example/api/` both resolve `chat.postMessage` to `https://sim.example/api/chat.postMessage`.
