...
- In the body, `ctx` has `session`, `callId`, `toolName`, `abortSignal`, `agent`, `agents`, and `ask`.
  ...
  `getSandbox` requires `sandbox: true` on the tool and runs only inside steps. `getSkill` remains unavailable.
...
Set `sandbox: true` to use `ctx.getSandbox()` in authored steps. eve opens or reconnects the
session sandbox before dispatching the workflow. Both blocking and background tools support
this option. Each step reconnects to the same sandbox; files written by an earlier step remain
...
step returns. `ctx.getSandbox()` throws in the workflow body or in a step without the opt-in.
