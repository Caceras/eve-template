...
Pass `model` or `instructions` to customize the default selector with serializable configuration. Pass an authored `select` step for full control, including live evaluation model instances:
...
A custom selector receives `(message, agents, abortSignal)` and must return one name from the provided map. `select` cannot be combined with `model` or `instructions`; configure those inside the selector.
