...
Use JEV when the parent model should decide whether to delegate, but your routing logic should choose the agent. Agents hidden from the parent model with `tool: false` or a same-named `disableTool()` remain present in `ctx.agents` and callable through `ctx.agent()`.

Automatic routing depends on agent descriptions. Give each candidate a description that distinguishes its responsibilities:

```ts title="agent/subagents/researcher/agent.ts"
import { defineAgent } from "eve";

export default defineAgent({
  description: "Investigate, analyze, and explain a topic.",
...
export default defineAgent({
  description: "Make operational changes and report the result.",
  model: "openai/gpt-5.6-sol",
  tool: false,
});
```

In a top-level root workflow, `ctx.agents` also contains `agent`, the root-copy target. To make the root copy a routing candidate, add a root description:

```ts title="agent/agent.ts"
import { defineAgent } from "eve";

export default defineAgent({
  description: "Coordinate work that needs the root agent's complete toolset.",
  model: "anthropic/claude-opus-4.8",
});
```

Without a root description, `ctx.agents.agent.description` is `""`. The provided routing helpers discard candidates whose descriptions are empty or contain only whitespace.

Choose the routing API based on how much of the tool you need to control:

1. Use `evaluate()` to define the candidate map, selection question, tool input, and invocation yourself.
2. Use `auto()` inside an authored step to reuse the standard selection logic while controlling the candidate map or using a live evaluation model.
3. Use `agentRouter()` for a complete workflow tool that routes across every described entry in `ctx.agents`.

#### Build the complete router

Use `evaluate()` directly when you need to control both selection and invocation:
...
async function chooseTarget(
  message: string,
  agents: Record<"researcher" | "operator", string>,
...
export default defineWorkflowTool({
  ...
  inputSchema: z.object({ message: z.string().min(1).max(8000) }),
...
The `"use step"` boundary makes the model call durable: workflow replay reuses the recorded choice instead of evaluating the route again. This example deliberately limits the candidates to `researcher` and `operator`; it does not include the root copy.

#### Reuse the standard selection logic

Use `auto()` from `eve/tools/agent-router/auto` inside your own authored step when the standard choice question is sufficient:

```ts
import { auto } from "eve/tools/agent-router/auto";
import { evaluationModel } from "../evaluation-model";

async function select(message: string, agents: Record<string, string>) {
  "use step";

  return auto({ message, agents, model: evaluationModel });
}
```

`auto()` requires `message` and `agents`. It accepts optional `model`, `instructions`, and `abortSignal` values. `model` may be an evaluation model instance or a model ID and defaults to `typesafe-ai/jev`. `auto()` does not create a durable step itself; call it from a function whose first statement is `"use step"`. Import or create a live model in that step's module rather than passing it into the step as a durable argument.

Before evaluation, `auto()` trims descriptions and removes empty entries. It returns the only candidate without calling the evaluation model, and otherwise returns the name selected by `evaluate()`. It chooses a name but does not invoke the agent.

#### Use the provided router tool

Use `agentRouter()` when the tool should route across every described candidate in `ctx.agents`. Export it from `agent/tools/agent.ts` to replace the model-facing built-in `agent` tool:
...
The tool accepts `{ message: string, outputSchema?: object }`. It removes candidates with empty descriptions, selects among the remaining names, and calls `ctx.agent()` with the original message and optional output schema. With one candidate, it invokes that candidate without evaluating a route.

By default, `agentRouter()` uses `typesafe-ai/jev`. You can pass a serializable model ID or custom instructions:
...
The `model` option accepts only a model ID. To use a live evaluation model or replace the selection logic, pass a top-level authored `"use step"` function as `select`:
...
export default agentRouter({ select });
```

The selector receives the message, the filtered description map, and an `AbortSignal` as its third argument. It must return a name from that map; `agentRouter()` rejects any other result. The selector is not called when only one described candidate remains. `select` cannot be combined with the factory's `model` or `instructions` options—configure the model and question inside the selector instead.

`ctx.agents` is a replay-stable metadata snapshot taken when the workflow starts. It includes declared agents even when their model-facing tools are hidden, but it does not expose model definitions, credentials, or callbacks. `agentRouter()` is available only to the top-level root session and is not inherited by delegated root copies or declared subagents, which prevents recursive routing through the same tool. `ctx.agent()` still checks the selected target's availability when the router invokes it.

When an authored tool accepts a JSON Schema supplied by the model, represent that input with a permissive object such as `z.looseObject({})`. Avoid `z.record(z.string(), z.json())`: its generated JSON Schema uses `propertyNames`, which OpenAI does not support. Validate the supplied value when your tool consumes it.

This pattern controls which agent handles a delegated task. It does not force the parent model to delegate. Route before the parent model runs if every request must go through JEV.
