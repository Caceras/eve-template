...
Pass `networkPolicy` to `create()` or `getOrCreate()`. The policy applies to the live VM, not the prepared snapshot. microsandbox accepts domain policies and credential transforms.

Session creation uses the recorded snapshot and image directly. Writable VM state persists through the provider's session snapshot lifecycle. A named shared VM exposes one filesystem and network boundary to every attached eve session.
