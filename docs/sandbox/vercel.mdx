...
`open()` also accepts Vercel compute resources, timeout, Drive mounts, and an optional `onSession` callback. Configure project credentials and custom fetch behavior on the environment, not in `open()`, because resume does not reconstruct start-only options. Vercel runs `onSession` only during the initial `start()` operation. Durable workflow steps reconnect by calling `resume()` directly, so they do not rerun `defineSandbox()`, `environment.open()`, or `onSession`.
