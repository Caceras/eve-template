...
Pass `networkPolicy` to `create()` or `getOrCreate()`. Docker supports only `"allow-all"` and `"deny-all"`. The policy applies to the live container, not the prepared image.

A session-owned container keeps writable `/workspace` state in its container filesystem. A stopped container restarts with that state. A named shared container exposes the same filesystem and network boundary to every attached eve session.
