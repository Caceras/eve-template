...
`environment.open()` creates or reconnects to the persistent sandbox owned by the current eve session. Pass options for the live sandbox to `open()`, then configure the returned sandbox before returning it:
    ...
    return {
      ...
      async open(ctx, source) {
        const remote = await client.getOrCreate({
          existingId: ctx.instance.kind === "restore" ? ctx.instance.metadata.remoteId : undefined,
...
`prepare()` returns a JSON-serializable artifact that eve stores by provider and template generation. The second `open()` argument is a required source union: `{ kind: "base" }` or `{ kind: "prepared", templateName, artifact }`. The fourth generic gives the prepared branch the provider's exact artifact type.

`ctx.options` contains options for the live sandbox. `ctx.instance` explicitly distinguishes first creation from restoration and carries typed persisted metadata only in the restore branch. Keep provider SDK values behind the provider so selectors always receive eve's `RuntimeSandboxSession`.
