...
The server also closes a live stream response on purpose after ten seconds without an event, so a long tool call, such as a sandbox query that runs for a minute, produces several short connections rather than one long one. This is invisible to your code: `for await`, `result()`, and `useEveAgent` continue uninterrupted, and no events are skipped because every event is durable and the next connection resumes from the cursor. The client treats the clean close as a cue to reconnect right away, without backoff, and it does not count toward the empty-stream limit. Only connections that end abnormally, such as a dropped socket or a read that stalls for twenty seconds, back off before reconnecting.
