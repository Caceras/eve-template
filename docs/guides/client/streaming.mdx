...
The server also closes a live stream response on purpose after ten seconds without an event, so a long tool call, such as a sandbox query that runs for a minute, produces several short connections rather than one long one. This is invisible to your code: `for await`, `result()`, and `useEveAgent` continue uninterrupted, and no events are skipped because every event is durable and the next connection resumes from the cursor. The server advertises the interval in the `x-eve-stream-idle-close-ms` header; when a connection ends cleanly after being quiet for that long, the client reconnects right away, without backoff, and does not count it toward the empty-stream limit. A stream that ends promptly, such as a settled session, still counts. Only connections that end abnormally, such as a dropped socket or a read that stalls for twenty seconds, back off before reconnecting.
