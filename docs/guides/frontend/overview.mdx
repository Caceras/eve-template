...
`useEveAgent()` prepares a session when the chat mounts and after each reset by default:
...
Pass `prewarm: false` to defer session creation until the first send.

Prewarming also sends the configured auth and headers before the first send. If your
application creates a chat record or prepares required credentials in its submit handler,
complete that setup before mounting the hook, or disable automatic prewarming. Use
`prewarm: false` for hook instances mounted only to render a read-only view without a
session. Channels that transform initialization identity in `onMessage` also need to
[account for the session initialization boundary](../../channels/eve#start-and-continue-a-session).

Prewarming creates the session, opens its event stream, and waits for the initial
`session.waiting` event. It leaves the composer at `"ready"` and does not run `prepareSend`.
If the user submits before initialization finishes, `send()` waits internally before posting the
message. The explicit `prewarm()` method remains available and resolves when initialization is
ready; concurrent calls share the same work. The lower-level `client.sessions.create()` still
resolves on `202 Accepted`.

If creation fails before a session is accepted, a waiting send falls back to creating a session
with its message. An initialization failure after acceptance is reported through `error` and
`onError`; call `reset()` to prepare a fresh session. Without a waiting send, a prewarm failure
sets `status` to `"error"`. The configured session timeout starts at creation.

The hook keeps consuming one session stream across turns and idle periods. Readiness, replies,
...
`reset()` reuses the hook instance and clears the conversation. By default, it immediately
prepares the next owned session; with `prewarm: false`, the next send creates it. An explicitly supplied
...
- `onError(error)`: fires when a turn or session stream fails, or prewarming fails with no send waiting.
