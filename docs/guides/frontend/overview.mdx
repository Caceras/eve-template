...
Pass `prewarm: true` to prepare a session when the chat mounts and after each reset:
...
`reset()` reuses the hook instance and clears the conversation. With `prewarm: true`, it immediately
prepares the next owned session; otherwise, the next send creates it. An explicitly supplied
