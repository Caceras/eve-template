...
| `prewarm` | Create and initialize the durable session without starting a turn.                        |
...
export function Chat() {
  ...
  useEffect(() => {
    void agent.prewarm().catch(() => {
      // The hook exposes the failure through agent.error.
    });
  }, [agent.prewarm]);
...
and does not open the event stream. A failed create sets `status` to `"error"` and populates
`error`. Concurrent calls share one creation request. If the user submits while creation is still
in flight, `send()` waits for the accepted session handle and sends the message through that
session. The configured session timeout starts when prewarming creates the session.
