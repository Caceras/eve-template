...
Call `prewarm()` when a chat page mounts to move durable session startup ahead of the user's first
message:
...
export function Chat() {
  ...
  useEffect(() => {
    void agent.prewarm().catch(() => {
      // The hook reports errors or retries creation if a send is waiting.
    });
  }, [agent.prewarm]);

  // Render the conversation and composer.
}
```

Prewarming keeps `status` at `"ready"` while the request is in flight, does not run `prepareSend`,
and does not open the event stream. The promise resolves when the server accepts creation;
initialization may still be running. A failed create rejects the promise and, when no send is
waiting, sets `status` to `"error"` and populates `error`. Concurrent calls share one creation
request. If the user submits while creation is still
in flight, `send()` waits for the accepted session handle and sends the message through that
session. If prewarming fails, the waiting send creates a session with its message instead and
reports an error only if that send fails. Calling `prewarm()` during a first send waits for its
session creation and rejects if creation fails. The configured session timeout starts when
prewarming creates the session.
...
- `onError(error)`: fires when a turn fails or prewarming fails with no send waiting.
