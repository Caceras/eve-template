...
- `t.prewarm(options?)` creates the session without starting a turn or consuming its stream. It resolves on `202 Accepted` and makes `t.sessionId` and `t.state` available. Options accept per-request `headers` and `signal`; creation uses the eval timeout signal by default.
...
Call `prewarm()` before the first message to exercise message-free creation through the eval driver:
...
export default defineEval({
  async test(t) {
    ...
    const turn = await t.send("Greet Alice briefly.");
...
Concurrent prewarm calls and a first send share session creation. A creation failure rejects
the waiting operations; a later call can retry. Once a session exists, `prewarm()` does nothing.
Independent sessions from `t.newSession()` expose the same method. Prewarmed sessions appear
in eval results and participate in timeout cleanup even when no message was sent.
